...
The handshake is a shared access password over a self-contained login page at `/dsh-auth-tunnel/login`: the password reference (never the value) lives in composition config; a successful POST mints `dsh_auth_tunnel`, an `HttpOnly; SameSite=Strict` cookie whose HMAC key is `SHA-256(password)`; a wrong password bounces with `?error=1`. Session keys resolve **per request**, so rotating the referenced credential invalidates every open session without a restart (the inline test composition proves this). `GET/POST /dsh-auth-tunnel/logout` clears the client-side cookie. Login posts are capped at 16 KiB (declared and streamed), a wrong content type answers 415, and the gate answers unauthenticated requests with 302 to the login page (navigation, `Sec-Fetch-Dest: document`, or `Accept: text/html`) or a terse 401 JSON (anything else, including upgrades). The one exception is read-only `/manifest.webmanifest`, whose browser fetch omits credentials without `crossorigin="use-credentials"`; it contains only public application metadata and is proxied for unauthenticated `GET`/`HEAD` requests. WebSocket/upgrade connections pass the same cookie check and then pipe raw bytes both ways, taking down their peer when one side closes. HTTP proxying likewise cancels the upstream request when the public client disconnects.

Mounting the bundle also replaces the boot-time native directory picker with the in-app directory browser. One boot can serve both loopback and public clients but can mount only one picker interaction; the browser interaction works for both, whereas a public `host.pickDirectory` call waits on an OS dialog on the Host until Cloudflare times out. A later profile patch may pin a different picker after disabling these two inserted rows, but doing so makes remote selection depend on the Host display again.
...
- **One directory-picker interaction per boot**: enabling this bundle uses the in-app browser for local clients too, because the Web app cannot mount native and browser pickers per connection.
