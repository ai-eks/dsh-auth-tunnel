...
The gate is the protection, so the row works purely against services: it reads `ctx.webServer.port` (`static inject = ['webServer']`, so a composition without the webserver leaves the row pending), resolves the password through the optional credentials service, and publishes facts into shell-env and the system prompt when those rows are mounted. No web-facing package changes shape. Behind the gate, accepted requests are proxied to the upstream with their `Host` rewritten to the loopback authority, which keeps the connection's DNS-rebinding trust fence (`/api` guards) reading the loopback surface it was built for. Direct use of `127.0.0.1:<webserver port>` stays unauthenticated on purpose: the password only locks the public path.
...
- **Shared-password, one user**: no rate limiting, no lockout, no per-user sessions, no server-side revocation list (password rotation invalidates everything); a webmail-grade deployment would front something stronger than this.
