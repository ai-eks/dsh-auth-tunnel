 ...
 */
class RemoteMutationLock {
  private busy = false
  private blocked = false
  private idleWaiters = new Set<() => void>()

  tryAcquire(): (() => void) | undefined {
    if (this.busy || this.blocked) return undefined
    this.busy = true
    let released = false
    return () => {
      if (released) return
...
class PasswordGate {
  ...
  private async handleRemoteSettings(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    const release = this.remoteMutations.tryAcquire()
    if (release === undefined) {
      writeJson(res, 409, { error: 'another plugin settings change is applying' })
      return
    }
    try {
      ...
      if (request.writes.length !== 0) {
        ...
        await settings.mutate(AUTH_TUNNEL_SETTINGS_NAMESPACE, ops, request.expectedRevision)
      } else if (request.password !== '') {
        await this.ctx.credentials.set(credentialRef(current.passwordRef), request.password)
      }
  ...
  private async handleRemoteLocale(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      await settings.mutate(
        ...
        openedSettings.descriptor.revision,
      )
...
class AuthTunnelRuntime {
  ...
  private readonly remoteMutations = new RemoteMutationLock()
  ...
  private async drain(): Promise<void> {
    try {
      while (this.desired !== undefined && !this.disposed) {
        ...
        this.desired = undefined
        try {
          await this.remoteMutations.waitForIdle()
          if (this.desired !== undefined) continue
      ...
      if (!this.disposed && this.desired === undefined) this.remoteMutations.unblock()
    }
  }
  ...
  private async reconcile(next: InternalConfig): Promise<void> {
    ...
    if (replaceGate) {
      ...
      if (previous !== undefined) {
        if (!candidate.alive) {
          const restored = this.restorePreviousAfterFailedSwap(candidate, previous)
          await this.stop(candidate)
          if (!restored) {
            this.active = undefined
            this.publish(undefined)
    ...
    if (replaceTunnel) {
      ...
      if (previous !== undefined) {
        if (!candidate.alive) {
          if (!this.restorePreviousAfterFailedSwap(candidate, previous)) {
            this.active = undefined
            this.publish(undefined)
  ...
  private async startFull(config: InternalConfig): Promise<ActiveTunnel> {
    ...
    try {
      ...
      return {
        config,
        tokenGeneration: this.tokenGeneration(config),
        gate,
        server: started.server,
        port: started.port,
        child: spawned.child,
  ...
  private restorePreviousAfterFailedSwap(candidate: ActiveTunnel, previous: ActiveTunnel): boolean {
    if (candidate.alive || !previous.alive || this.disposed) return false
    const failure = this.status.message ?? 'auth-tunnel: replacement cloudflared exited before the previous tunnel was retired.'
    const auth = this.configured
    if (auth !== undefined) {
      previous.config = {
        ...previous.config,
