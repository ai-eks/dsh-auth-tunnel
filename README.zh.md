...
保护在门上,因此该行只针对服务工作:读取 `ctx.webServer.port`(`static inject = ['webServer']`,组合里没有 webserver 时行保持 pending),通过可选的 credentials 服务解析密码,并在 shell-env 和 system-prompt 行挂载时注入事实。任何面向 Web 的包都不需要改动。门后,被放行的请求会被代理到上游,`Host` 改写为 loopback 地址——connection 的 DNS-rebinding 信任栅栏(`/api` 防护)看到的仍是它为之构建的 loopback 面。直接访问 `127.0.0.1:<webserver 端口>` 是有意的未认证:密码只锁公网路径。
...
- **共享密码、单用户**:无速率限制、无锁定、无按用户会话、无服务端吊销表(密码轮换会全量失效);更严肃的部署应当换更强的前置认证。
